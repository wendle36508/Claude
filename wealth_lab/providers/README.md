...
against Finnhub's `/quote`, `/company-news`, and `/stock/profile2`
endpoints - not a sketch. It's registered lazily (see `providers/__init__.py`)
so the default mock-only path never needs the `requests` package installed.
