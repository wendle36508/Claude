...
CORE_ALLOCATIONS = [
    # (symbol, name, thesis, entry_price, sleeve, target_weight)
    ("SPY", "SPDR S&P 500 ETF Trust", "Broad US equity core; the ballast that keeps this from being an AI-only bet.", 761.69, "core_equity", 0.30),
    ("BND", "Vanguard Total Bond Market ETF", "Core fixed income; dampens volatility and is the standard multi-asset diversifier a stock-only portfolio lacks.", 71.94, "core_bond", 0.15),
    ("CASH", "Cash", "Liquidity buffer for rebalancing and to avoid being forced to sell into a drawdown.", None, "cash", 0.05),
]
...
def run() -> None:
    with db.connect() as conn:
        ...
        for symbol, name, thesis_text, entry_price, sleeve, weight in CORE_ALLOCATIONS:
